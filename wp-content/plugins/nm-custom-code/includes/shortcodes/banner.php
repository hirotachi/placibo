	...
	function nm_shortcode_banner( $atts, $content = NULL ) {
		...
		if ( strlen( $image_id ) > 0 ) {
			...
			if ( $image_type == 'fluid' ) {
				...
				$image_output .= apply_filters( 'nm_banner_img_output', '<img src="' . esc_url( $image[0] ) . '" alt="' . esc_attr( $image_title ) . '" />', $image_id );
				...
				if ( strlen( $alt_image_id ) > 0 ) {
                    ...
                    $image_output .= apply_filters( 'nm_banner_alt_img_output', '<img src="' . esc_url( $alt_image[0] ) . '" class="nm-banner-alt-image img" alt="' . esc_attr( $alt_image_title ) . '" />', $alt_image_id );
				}
			} else {
				// Image height style
		...
		$link_class = '';
		if ( strlen( $link ) > 0 ) {
			if ( $link_is_custom && function_exists( 'nm_build_link' ) ) {
